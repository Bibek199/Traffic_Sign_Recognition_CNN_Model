...
TestSet=pd.read_csv("Test_Robot_sift.csv", usecols = ['Path','ClassId'])
...
ref_img00=cv2.cvtColor(cv2.imread(Path+Ref00),cv2.COLOR_RGB2GRAY)
ref_img03=cv2.cvtColor(cv2.imread(Path+Ref03),cv2.COLOR_RGB2GRAY)
ref_img14=cv2.cvtColor(cv2.imread(Path+Ref14),cv2.COLOR_RGB2GRAY)
ref_img33=cv2.cvtColor(cv2.imread(Path+Ref33),cv2.COLOR_RGB2GRAY)
ref_img34=cv2.cvtColor(cv2.imread(Path+Ref34),cv2.COLOR_RGB2GRAY)
...
def Calculate_Threshold(ref_img,Train,Path):
    ...
    for n in range(len(Train)):
        ...
        img = cv2.cvtColor(cv2.imread(Path + img_path), cv2.COLOR_RGB2GRAY)
...
def Calculate_Confusion_Matrix(ref_img,Ref_ID,TestSet,Path,Thr):
    ...
    for n in range(len(TestSet)):
        ...
        imgTest=cv2.cvtColor(cv2.imread(Path+imgTest_path),cv2.COLOR_RGB2GRAY)
        kp_imgTest,des_imgTest=sift.detectAndCompute(imgTest, None)
